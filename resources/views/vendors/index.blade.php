...
<head>
    ...
    <title>Vendor - SIAM</title>

    <link rel="icon" href="{{ asset('images/fav_icon.png') }}" type="image/png">
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>

<body class="bg-slate-100 font-sans" x-data="vendorManager()">

    <x-header title="Vendor" />
    <x-sidebar />
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-gray-500">Vendor sewa & pembelian aset IT</p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <form method="GET" action="{{ route('siam.vendors.index') }}"
                    class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        <label class="block text-xs text-gray-500 mb-1">Cari Nama</label>
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Contoh: Lenovo" class="w-full border rounded-lg px-3 py-2 text-sm">
                    </div>
                    <div>
                        ...
                        <select name="type" data-auto-submit class="w-full border rounded-lg px-3 py-2 text-sm">
                            ...
                            <option value="sewa" @selected(request('type') === 'sewa')>Sewa</option>
                            <option value="pembelian" @selected(request('type') === 'pembelian')>Pembelian</option>
                            <option value="both" @selected(request('type') === 'both')>Sewa & Beli</option>
                        </select>
                    </div>
                            ...
                            @forelse ($vendors as $i => $v)
                                @php
                                    $vendorData = [
                                        ...
                                        'assets_count' => $v->asset_ownerships_count,
                                        'routes' => [
                                            'edit' => route('siam.vendors.edit', $v),
                                            'delete' => route('siam.vendors.destroy', $v),
                                        ],
                                    ];
                                @endphp
                                <tr @click='openModal({{ json_encode($vendorData, JSON_HEX_APOS | JSON_HEX_QUOT) }})'
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @endphp
                                        <span class="px-2 py-0.5 text-xs rounded {{ $typeColor }} font-medium">
                                            {{ $v->type_label }}
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-xs">{{ $v->contact_person ?? '-' }}</td>
                                    <td class="px-4 py-3 text-xs">
                                        @if ($v->phone)
                                            <div>📞 {{ $v->phone }}</div>
                                        @endif
                                        @if ($v->email)
                                            <div class="text-gray-500">✉️ {{ $v->email }}</div>
                                        @endif
                                        @if (!$v->phone && !$v->email)
                ...
                <div class="flex items-start justify-between">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                            ...
                            <span class="text-xs px-2 py-0.5 rounded bg-white/20 backdrop-blur-sm font-semibold"
                                x-text="selected?.assets_count + ' aset'"></span>
                        </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-b">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        ...
                        <div class="text-xs" x-text="selected?.contact_person ?? '-'"></div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-500">Telepon</div>
                        <div class="text-xs" x-text="selected?.phone ?? '-'"></div>
                    </div>
                    <div class="col-span-2">
                        <div class="text-xs text-gray-500">Email</div>
                        <div class="text-xs" x-text="selected?.email ?? '-'"></div>
                    </div>
                    <div class="col-span-2">
