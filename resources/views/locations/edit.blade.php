    ...
    <div x-data="pageLayout()" :class="collapsed ? 'lg:ml-16' : 'lg:ml-64'"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Edit Lokasi</h1>
                    <p class="text-sm text-gray-500 mt-0.5">
                        Perbarui data <strong>{{ $location->full_name ?? $location->room }}</strong>
                    </p>
                </div>
            </div>
            ...
            <form action="{{ route('siam.locations.update', $location) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="room" value="{{ old('room', $location->room) }}" required
                            class="w-full border rounded-xl px-3 py-2.5 text-sm
                                   focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                    ...
                    <div>
                        ...
                        <select name="division"
                            ...
                            @foreach ($departments as $d)
                                <option value="{{ $d->name }}" @selected(old('division', $location->division) === $d->name)>
                                    {{ $d->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3"
                        class="w-full border rounded-xl px-3 py-2.5 text-sm
                               focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                               border-gray-200">{{ old('notes', $location->notes) }}</textarea>
                </div>
