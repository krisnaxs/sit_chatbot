...
<body class="bg-slate-100 font-sans" x-data="userForm()">
    ...
    <div x-data="pageLayout()" :class="collapsed ? 'lg:ml-16' : 'lg:ml-64'"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            {{-- INFO USERNAME --}}
            <div class="mb-6 p-3 rounded-xl bg-slate-50 border border-slate-200 flex items-start gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-slate-500 shrink-0 mt-0.5" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <div class="text-xs text-slate-700">
                    <span class="font-semibold">Username saat ini:</span>
                    <code class="font-mono text-indigo-700">{{ $user->username }}</code>
                    <span class="text-slate-500">(otomatis di-regenerate kalau email berubah)</span>
                </div>
            </div>
                    ...
                    <div class="mt-4">
                        ...
                        <select name="location_id"
                            ...
                            @foreach ($locations as $l)
                                <option value="{{ $l->id }}" @selected(old('location_id', $user->location_id) == $l->id)>
                                    {{ $l->full_name }}
                                </option>
                            @endforeach
                        </select>
