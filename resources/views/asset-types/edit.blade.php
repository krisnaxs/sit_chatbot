...
<head>
    ...
    <title>Edit Brand & Model - SIAM</title>
...
<body class="bg-slate-100 font-sans">
    ...
    <x-header title="Edit Brand & Model" />
    <x-sidebar />
    ...
    <div x-data="pageLayout()" :class="collapsed ? 'lg:ml-16' : 'lg:ml-64'"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
            ...
            <div class="flex items-center gap-4 mb-6 pb-6 border-b border-gray-100">
                ...
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Edit Brand & Model</h1>
                    <p class="text-sm text-gray-500 mt-0.5">
                        Perbarui <strong>{{ $assetType->brand }} — {{ $assetType->model }}</strong>
                    </p>
                </div>
            </div>
            ...
            <form action="{{ route('siam.asset-types.update', $assetType) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="brand" value="{{ old('brand', $assetType->brand) }}" required
                        list="brandList" placeholder="Pilih atau ketik brand baru..."
                ...
                <div>
                    ...
                    <input type="text" name="model" value="{{ old('model', $assetType->model) }}" required
                        class="w-full border rounded-xl px-3 py-2.5 text-sm
                                  focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                    @error('model')
                        <p class="text-xs text-red-500 mt-1.5">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full border rounded-xl px-3 py-2.5 text-sm
                                     focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ old('description', $assetType->description) }}</textarea>
                </div>
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer select-none">
                        <input type="checkbox" name="is_active" value="1"
                            ...
                            {{ old('is_active', $assetType->is_active) ? 'checked' : '' }}>
                        <div>
